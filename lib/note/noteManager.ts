...
export const addBlocks = async (noteID: string, content: string) => {
...
const getFolderCollection = async () => {
  ...
  // if (!userData.primaryEmailAddress) {
...
export const getFolders = async () => {
  const folderCollection = await getFolderCollection();
  const folders = (await folderCollection.get()).docs.map((folderDoc: FirebaseFirestore.QueryDocumentSnapshot) =>
    folderDoc.data(),
  );

  const folderObjs = await Promise.all(
    folders.map(async (folder: FirebaseFirestore.DocumentData) => {
...
function parseBlocks(noteID: string, content: string): Block[] {
  const blocks: Block[] = [];
  let currentBlockContent: ContentNode[] = [];
  let order = 0;
